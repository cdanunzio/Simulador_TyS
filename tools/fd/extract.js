...
(async () => {
  ...
  const data = await page.evaluate(() => {
    ...
    const roles = m.roles.map(r => ({ id: r.id, nombre: r.nombre, etapa: r.etapa, nota: r.nota || '', etapas: m.estados.filter(e => e.responsable === r.id).map(e => e.nombre), perm: resumenPermisos(r.id), modulos: m.modulos.filter(x => !moduloHabilitadoRol(x.id, r.id)).map(x => x.nombre.split(' (')[0]) }));
