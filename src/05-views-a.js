...
function viewInicio() {
  ...
  return pageH('Inicio', ctxTxt(), rol === 'COM' ? btn('Nueva orden de servicio', 'go', { screen: 'nueva' }, 'pri') : rol === 'LAR' ? btn('Logística de arribo', 'go', { screen: 'arribos' }, 'pri') : rol === 'MD' ? btn('Datos maestros', 'go', { screen: 'md' }, 'pri') : rol === 'ARE' ? btn('Mi área', 'go', { screen: 'area' }, 'pri') : '') +
    ...
    (moduloHabilitado('arribos') ? '<div class="card"><div class="card-h"><h2>Próximos arribos</h2>' + btn('Logística de arribo', 'go', { screen: 'arribos' }, 'sm ghost') + '</div>' +
...
function viewExpediente(o) {
  ...
  if (ap) {
    ...
    let fueraTol = false;
    if (o.estado === 'PEND_CIERRE' && mine) { const CZ = S.ctx.cz && S.ctx.cz.id === o.id ? S.ctx.cz : null; const ev = evaluarMerma(o); fueraTol = !ev.dentro && !CZ?.aprob; }
...
function secEjecucion(o) {
  ...
  if (!ex) {
    ...
    const m = metricasReal(o); const pct = o.toneladas ? ex.acumulado / o.toneladas : 0; const activo = o.estado === 'EJEC' && rol === 'OPS'; const gestiona = puedeEjecutar(o, rol);
    ...
    body = '<div class="meter" style="margin-bottom:6px">' + bar(pct, pct >= 1 ? 'ok' : '') + '<b class="num">' + fmtT(ex.acumulado) + ' / ' + (o.toneladas ? fmtT(o.toneladas) : '—') + ' t · ' + fmtPct(pct) + '</b></div>' +
      ...
      (!activo && gestiona ? '<div class="btn-row" style="margin-bottom:12px">' + btn('Agregar recurso de depósito', 'recurso-form', { id: o.id }, 'pri') + '<span class="small muted">Operaciones y Depósito trabajan el operativo en simultáneo: Depósito asigna o libera los recursos de su ámbito y los compartidos mientras se descarga ' + sup('S35') + '.</span></div>' : '') +
      (activo ? '<div class="btn-row" style="margin-bottom:12px">' + btn('Simular 1 hora', 'simular', { id: o.id, h: 1 }, '') + btn('Simular turno (6 h)', 'simular', { id: o.id, h: 6 }, '') + btn('Registrar ticket', 'ticket-form', { id: o.id }, '') + btn('Agregar recurso', 'recurso-form', { id: o.id }, 'pri') + btn('Registrar demora', 'demora-form', { id: o.id }, '') + btn('Calidad de la mercadería', 'calidad-form', { id: o.id }, '') + '<span class="small muted">La maqueta simula tickets de balanza según el ritmo de los equipos activos.</span></div>' : '') +
      alertBox(o.calidadRegistro ? 'ok' : 'warn', '<div><b>Calidad de la mercadería:</b> ' + (o.calidad ? '<b>' + esc(o.calidad) + '</b>' : '<i>sin registrar</i>') + ' · ' + (o.calidadRegistro ? 'registrada por ' + esc(o.calidadRegistro.por) + ' el ' + fmtDT(o.calidadRegistro.ts) + (o.calidadRegistro.anterior ? ' (antes: ' + esc(o.calidadRegistro.anterior) + ')' : '') + (o.calidadRegistro.motivo ? ' · ' + esc(o.calidadRegistro.motivo) : '') : 'viene declarada en el origen; <b>Operaciones debe registrar la calidad efectiva</b> ' + sup('S28')) + '</div>') +
      '<h3 style="margin:10px 0 6px">Recursos activos y modificaciones <span class="small muted" style="font-weight:400">alta · modificación · reemplazo · baja, con registro para la comparativa</span></h3>' + ambLeyenda() +
      table([{ h: 'Recurso', f: r => esc(recNombre(r.rid)) + (r.cantidad > 1 ? ' <span class="tag">×' + r.cantidad + '</span>' : '') }, { h: 'Ámbito', f: r => ambChip(r.rid) }, { h: 'Origen', f: r => r.origen === 'plan' ? chip('Plan inicial', '') : r.origen === 'reemplazo' ? chip('Reemplazo de ' + (r.reemplazaA || ''), 'warn') : r.origen === 'modificado' ? chip('Cantidad ' + (r.cantidadAnterior || '') + ' → ' + r.cantidad, 'warn') : chip('Adicional', 'warn') }, { h: 'Desde → hasta', f: r => '<span class="mono xs">' + fmtDT(r.desde) + ' → ' + (r.hasta ? fmtDT(r.hasta) : 'activo') + '</span>' }, { h: 'Horas', cls: 'num', f: r => fmtN(horasRecurso(o, r), 1) }, { h: 'Motivo · responsable', f: r => (r.origen !== 'plan' ? esc(r.motivo) + '<br><span class="xs muted">' + esc(r.responsable) + (r.rol ? ' (' + esc(rolName(r.rol)) + ')' : '') + (r.respaldo ? ' · ' + esc(r.respaldo) : '') + '</span>' : '') + (r.motivoLiberacion ? '<br><span class="xs muted">Baja: ' + esc(r.motivoLiberacion) + '</span>' : (r.origen === 'plan' ? '<span class="dim">—</span>' : '')) }, { h: 'Cargo al cliente', f: r => r.origen !== 'plan' ? (r.atribuibleCliente ? chip(fmtUSD(cargosDe(o).find(c => c.ref === 'rec:' + ex.recursos.indexOf(r))?.monto || 0) + ' · ' + (r.aprobacion || 'pendiente'), r.aprobacion === 'Aprobado' ? 'ok' : r.aprobacion === 'Rechazado' ? 'crit' : 'warn') : chip('No atribuible', '')) : '<span class="dim">—</span>' }, { h: 'ABM', f: r => gestiona && !r.hasta && puedeGestionarRecurso(r.rid, rol) ? '<div class="btn-row" style="gap:3px">' + (['logistica', 'mano', 'funcion'].includes(r.tipo) ? btn('Modificar', 'modificar-form', { id: o.id, idx: ex.recursos.indexOf(r) }, 'sm') : '') + btn('Reemplazar', 'reemplazar-form', { id: o.id, idx: ex.recursos.indexOf(r) }, 'sm') + btn('Liberar', 'liberar-form', { id: o.id, idx: ex.recursos.indexOf(r) }, 'sm') + '</div>' : '' }], ex.recursos, { cls: 'compact' }) +
...
function secDeposito(o) {
  ...
  if (usaDeposito(o)) {
    ...
    if (o.estado === 'EJEC') {
      const ex2 = o.ejecucion; const mios = (ex2?.recursos || []).filter(r => !r.hasta && ambitoRecurso(r.rid) !== 'operaciones');
      body += alertBox('info', '<div>Operativo en curso: <b>Operaciones y Depósito trabajan la orden en simultáneo</b> ' + sup('S35') + '. Mientras se descarga el buque, Depósito recibe la mercadería y puede asignar o liberar los recursos de su ámbito y los compartidos, sin esperar el traspaso formal.</div>') +
        '<h3 style="margin:14px 0 6px">Recursos del ingreso <span class="small muted" style="font-weight:400">ámbito Depósito y compartidos</span></h3>' + ambLeyenda() +
        table([{ h: 'Recurso', f: r => esc(recNombre(r.rid)) + (r.cantidad > 1 ? ' <span class="tag">×' + r.cantidad + '</span>' : '') }, { h: 'Ámbito', f: r => ambChip(r.rid) },
          { h: 'Desde', f: r => '<span class="mono xs">' + fmtDT(r.desde) + '</span>' }, { h: 'Origen', f: r => r.origen === 'plan' ? chip('Plan inicial', '') : chip('Agregado en ejecución', 'warn') },
          { h: 'Responsable', f: r => esc(r.responsable || '—') + (r.rol ? '<br><span class="xs muted">' + esc(rolName(r.rol)) + '</span>' : '') },
          { h: 'ABM', f: r => rol === rc && puedeGestionarRecurso(r.rid, rol) ? '<div class="btn-row" style="gap:3px">' + (['logistica', 'mano', 'funcion'].includes(r.tipo || recursoTipo(r.rid)) ? btn('Modificar', 'modificar-form', { id: o.id, idx: ex2.recursos.indexOf(r) }, 'sm') : '') + btn('Liberar', 'liberar-form', { id: o.id, idx: ex2.recursos.indexOf(r) }, 'sm') + '</div>' : '' }],
          mios, { cls: 'compact', empty: 'El operativo todavía no tiene recursos de depósito asignados.' }) +
        (rol === rc ? '<div class="btn-row" style="margin-top:8px">' + btn('Agregar recurso de depósito', 'recurso-form', { id: o.id }, 'pri') + btn('Ver la ejecución completa', 'go-sec', { id: o.id, sec: 'ejecucion' }, '') + '</div>' : '');
    }
  } else {
  ...
  if (o.estado === 'PEND_CIERRE') {
    ...
    const difTxt = ev.merma ? '<b>merma de ' + fmtN(ev.merma, 1) + ' t</b>' : ev.excedente ? '<b>excedente de ' + fmtN(ev.excedente, 1) + ' t</b>' : '<b>sin diferencia</b>';
    body += '<h3 style="margin:14px 0 8px">Cierre del operativo</h3>' + cierreResumen(o) +
      (rol === rc ? '<div class="card tight" style="margin-top:10px;border-color:var(--accent-line)"><div class="card-h"><h3>Merma o excedente según balanza ' + sup('S27') + '</h3><span class="small muted">Tolerancia contractual: <b>' + fmtN(ev.tol, 1) + ' %</b> sobre ' + fmtT(ev.previsto) + ' t previstas' + (o.contrato?.condiciones?.toleranciaMermaPct != null ? ' (' + esc(o.contrato.id) + ')' : ' (parámetro general)') + '</span></div>' +
        alertBox('info', '<div>La diferencia <b>no se carga a mano</b>: surge de lo que declara la balanza al finalizar el operativo. Previsto <b>' + fmtT(ev.previsto) + ' t</b> · pesado <b>' + fmtT(ev.pesado) + ' t</b> en ' + ev.tickets + ' tickets → ' + difTxt + '.</div>') +
        table([{ h: 'Concepto', f: x => x[0] }, { h: 'Toneladas', cls: 'num', f: x => x[1] }, { h: 'Origen del dato', f: x => '<span class="small muted">' + x[2] + '</span>' }],
          [['Previsto en la orden', fmtT(ev.previsto) + ' t', 'Datos del servicio (Comercial)'], ['Pesado en balanza', fmtT(ev.pesado) + ' t', ev.tickets + ' tickets de balanza de la ejecución'], [ev.excedente ? 'Excedente' : 'Merma', fmtN(ev.merma || ev.excedente, 1) + ' t', 'diferencia calculada · ' + fmtN(ev.pct, 2) + ' % sobre lo previsto']], { cls: 'compact' }) +
        (ev.merma || ev.excedente ? alertBox(ev.dentro ? 'ok' : 'crit', '<div><b>' + fmtN(ev.pct, 2) + ' %</b> sobre lo previsto · ' + (ev.dentro ? 'dentro de la tolerancia contractual (' + fmtN(ev.tol, 1) + ' %): el operativo puede cerrarse.' : 'FUERA de la tolerancia contractual (' + fmtN(ev.tol, 1) + ' %): el cierre requiere aprobación de Comercial.') + '</div>') : alertBox('ok', 'La balanza no arroja diferencia contra lo previsto.')) +
