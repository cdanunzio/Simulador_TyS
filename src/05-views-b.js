...
function flotaBlock(o, R, E) {
  ...
  return '<div><h3 style="margin:6px 0">Logística (flota)</h3><div class="stack" style="gap:6px">' +
    ...
    '</div><p class="help">Flota: <b>camión propio</b> (de la empresa) y <b>camión contratado</b> (transportista). Se planifica por unidad completa.</p></div>';
}
function maquinariaBlock(o, R, E) {
  const list = logisticaDe(E, 'maquinaria').filter(mdUsable); const dest = R.deposito || null;
  /* lista desplegable con el detalle de cada unidad: se elige y se agrega (revisión 17/09) */
  const opciones = []; const elegidas = [];
  for (const l of list) {
    const uds = unidadesDe(l.id); const sel2 = unidadesElegidas(R, l.id);
    if (!uds.length) { /* maquinaria sin unidades cargadas: se asigna por cantidad */
      const n = R.logistica?.[l.id] || 0;
      elegidas.push({ maq: l, unidad: null, n, pct: R.maqPct?.[l.id] ?? 100 });
      continue;
    }
    for (const u of uds) {
      if (sel2.includes(u.id)) { elegidas.push({ maq: l, unidad: u, n: 1, pct: R.maqPct?.[l.id] ?? 100 }); continue; }
      const ap = unidadApta(u, dest);
      opciones.push({ v: l.id + ':' + u.id, grupo: l.nombre, t: u.interno + ' · ' + u.marca + ' ' + u.modelo + ' ' + u.anio + ' · ' + u.capacidad + ' · ' + fmtN(u.anchoM, 1) + ' × ' + fmtN(u.altoM, 1) + ' m' + (ap.apta ? '' : ' — ' + ap.motivo), dis: !ap.apta });
    }
  }
  const grupos = [...new Set(opciones.map(x => x.grupo))];
  const selector = '<div class="btn-row" style="align-items:flex-end;gap:8px">' +
    field('Agregar maquinaria', '<select id="pf-maq-sel">' + '<option value="">— elegí una unidad —</option>' +
      grupos.map(g => '<optgroup label="' + esc(g) + '">' + opciones.filter(x => x.grupo === g).map(x => '<option value="' + esc(x.v) + '"' + (x.dis ? ' disabled' : '') + '>' + esc(x.t) + (x.dis ? ' 🚫' : '') + '</option>').join('') + '</optgroup>').join('') +
      '</select>', '', 'style="flex:1 1 320px;min-width:0"') +
    btn('Agregar', 'pf-maq-add', {}, 'pri sm') + '</div>';
  const filas = elegidas.length ? '<div class="stack" style="gap:6px;margin-top:8px">' + elegidas.map(x => {
    const usados = usoPico(x.maq.id, o); const libre = Math.max(0, x.maq.cantidad - usados - x.n * x.pct / 100);
    return '<div class="eq sel"><div style="display:flex;gap:8px;align-items:flex-start;flex-wrap:wrap">' +
      '<div style="flex:1 1 260px;min-width:0"><b>' + esc(x.maq.nombre) + '</b> ' + ambChip(x.maq.id) +
      (x.unidad ? '<br><span class="small">' + esc(x.unidad.interno) + ' <span class="tag">' + esc(x.unidad.id) + '</span> ' + esc(x.unidad.marca + ' ' + x.unidad.modelo + ' ' + x.unidad.anio) + ' · ' + esc(x.unidad.capacidad) + ' · ' + fmtN(x.unidad.anchoM, 1) + ' × ' + fmtN(x.unidad.altoM, 1) + ' m</span>' +
        '<br><span class="xs muted">' + esc(x.unidad.ubicacion_habitual) + ' · ' + fmtUSD(x.maq.costoHora) + '/h · ' + esc(buName(x.maq.bu)) + (dest ? ' · ' + (unidadApta(x.unidad, dest).apta ? 'entra en ' + esc(recNombre(dest)) : '<span class="crit">no entra en ' + esc(recNombre(dest)) + '</span>') : '') + '</span>'
        : '<br><span class="xs muted">' + x.n + ' unidad' + (x.n > 1 ? 'es' : '') + ' · ' + fmtUSD(x.maq.costoHora) + '/h · ' + esc(buName(x.maq.bu)) + ' · sin inventario por unidad</span>') + '</div>' +
      '<label class="field" style="flex:0 0 108px"><span>% de uso</span><input type="number" min="1" max="100" step="5" data-pf="maqpct:' + x.maq.id + '" value="' + x.pct + '"></label>' +
      '<div style="align-self:center">' + (x.unidad ? btn('Quitar', 'pf-maq-del', { u: x.maq.id + ':' + x.unidad.id }, 'sm danger') : btn('Quitar', 'pf-maq-del', { u: x.maq.id + ':' }, 'sm danger')) + '</div>' +
      '</div>' + (x.pct < 100 ? '<span class="xs">' + chip('remanente ' + (100 - x.pct) + ' % disponible para otra orden', 'info') + '</span>' : '') +
      (libre > 0 ? '<span class="xs muted">libre en la ventana: ' + fmtN(libre, 1) + ' de ' + x.maq.cantidad + '</span>' : '') + '</div>';
  }).join('') + '</div>' : '<p class="muted small" style="margin-top:8px">Sin maquinaria asignada. Elegí una unidad de la lista y agregala.</p>';
  return '<div><h3 style="margin:6px 0">Maquinaria <span class="tag">' + elegidas.length + ' asignada' + (elegidas.length === 1 ? '' : 's') + '</span></h3>' +
    (dest ? '<p class="help" style="margin:0 0 6px">Acceso del destino <b>' + esc(recNombre(dest)) + '</b>: ' + (accesoDe(dest) ? fmtN(accesoDe(dest).anchoM, 1) + ' × ' + fmtN(accesoDe(dest).altoM, 1) + ' m (' + esc(accesoDe(dest).tipo) + ')' : 'sin restricción declarada') + '. Las unidades que no entran aparecen deshabilitadas en la lista.</p>' : '') +
    selector + filas +
    '<p class="help" style="margin-top:8px">Cada unidad se afecta con un <b>% de uso</b> para esta orden; el remanente queda disponible para otro operativo. La misma máquina tiene unidades con distintas medidas y capacidades: la lista muestra el detalle y solo deja elegir las que entran por el acceso del destino.</p></div>';
}
...
let W = null;
function wInit(o) {
  W = o ? { edit: o.id, entidad: o.entidad, servicio: o.servicio, bu: o.bu || 'ENT', medio: o.medio, dest: o.destinatario ? o.destinatario.tipo + ':' + o.destinatario.id : '', producto: o.producto || (srv(o.servicio)?.requiereProducto === false ? 'NA' : ''), presentacion: o.presentacion || presentacionSugerida(o.producto), instrumento: o.instrumento || '', origen: o.origen ? o.origen.tipo + ':' + o.origen.id + ':' + (o.origen.cargaIdx ?? '') : (o.detalle ? 'detalle::' : ''), toneladas: o.toneladas, ventana: clone(o.ventana), nacionalizada: !!o.habilitaciones.nacionalizada, nacRef: o.habilitaciones.nacRef || '', msConfirmado: !!o.habilitaciones.msConfirmado, notas: o.notas || '', det: o.detalle ? clone(o.detalle) : wDetDefault(srv(o.servicio)?.detalle, o.entidad) }
    : { edit: null, entidad: S.ctx.entidad === 'ALL' ? 'TYS' : S.ctx.entidad, servicio: '', bu: '', medio: '', dest: '', producto: '', presentacion: '', instrumento: '', origen: '', toneladas: 0, ventana: null, nacionalizada: false, nacRef: '', msConfirmado: false, notas: '', det: null };
}
...
function wOrigenes() {
  ...
  if (m.origen === 'lineup') for (const lu of S.ops.lineups.filter(l => l.terminal === E && l.estado !== 'Zarpó')) lu.cargas.forEach((c, i) => {
    if (!c.bl || !(c.toneladas > 0)) return; /* bodega vacía: todavía no hay carga que nominar */
    if (c.operador && !esOperadorPropio(c.operador)) return; /* la opera otro: no es nominable */
    if (d?.tipo === 'cliente' && c.cliente !== d.id) return; if (P && c.producto !== P) return;
    const os = ordenesDeOrigen('lineup', lu.id, i).filter(o => o.id !== W.edit);
    out.push({ v: 'lineup:' + lu.id + ':' + i, t: lu.buque + ' · bodega ' + (c.bodega || i + 1) + ' · ' + c.bl + ' · ' + fmtT(c.toneladas) + ' t · ' + puertoNombre(c.puertoDescarga) + ' · ETB ' + fmtDT(lu.etb) + (c.operador ? '' : ' · sin operador asignado') + (os.length ? ' · ya vinculada a ' + os.map(o => o.id).join(', ') : '') });
  });
...
function wSpec() {
  ...
  return { id: W.edit || undefined, entidad: W.entidad, bu: W.bu === 'ENT' ? null : W.bu, medio: W.medio, servicio: W.servicio, destinatario: d, producto: W.producto && W.producto !== 'NA' ? W.producto : null, instrumento: W.instrumento || null,
    origen: tipo && tipo !== 'detalle' ? { tipo, id, cargaIdx: idx === '' ? undefined : +idx } : null, detalle: srv(W.servicio)?.detalle && W.det ? Object.assign({}, W.det, W.det.tipo === 'logistica' ? { km: kmEntre(W.det.origen, W.det.destino) } : {}) : null, toneladas: +W.toneladas || 0, ventana: W.ventana, notas: W.notas, presentacion: W.presentacion || presentacionSugerida(W.producto),
...
function viewNueva() {
  ...
  let html = pageH(W.edit ? 'Editar borrador ' + esc(W.edit) : 'Nueva orden de servicio', 'Selección secuencial: <b>Entidad → Servicio</b> y, según el servicio, el resto de los datos. Cada elección filtra y habilita la siguiente; al modificar un dato previo se revisan las selecciones dependientes.') + steps +
    ...
    (W.producto && W.producto !== 'NA' ? field('Presentación de la mercadería', sel('w-presentacion', presentacionesDe(W.producto).map(x => ({ v: x, t: x })), W.presentacion || presentacionSugerida(W.producto), 'data-w="presentacion"')) : '') +
...
function adminModulos() {
  ...
  return '<div class="stack">' + simCard +
    '<div class="card"><div class="card-h"><h2>Por rol / sector ' + sup('S20') + '</h2><span class="small muted">todos los módulos · Inicio no se puede deshabilitar</span></div>' + matriz('rol', m.roles, m.modulos, r => esc(r.nombre)) + '</div>' +
...
function viewArea() {
  ...
  const head = pageH('Mi área · ' + esc(a.nombre), 'Capacidad total del sector, ABM de sus recursos (por workflow de master data) y reservas para operativos futuros ' + sup('S23'),
    ...
    '<div class="form-grid" style="margin-bottom:10px">' + areaSelector(a) + field('Responsable', '<input value="' + esc(a.responsable || rolName('ARE')) + '" disabled>') + field('Departamento · BU', '<input value="' + esc((byId(md().departamentos, a.departamento)?.nombre || '—') + ((a.bus || []).length ? ' · ' + a.bus.map(buName).join(', ') : '')) + '" disabled>') + '</div>' +
